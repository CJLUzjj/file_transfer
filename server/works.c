...
char *mbegin[CONN_MAX];

//创建管道,pipe in main function
int pipefd[2];
...
void * sender(void *args){
    ...
    char *fp = mbegin[p_fhead->id]+p_fhead->offset;
    for(int i=0;i <num; i++){
        if((send_size = send(fd, fp, SEND_SIZE, 0)) == SEND_SIZE){
            fp+=SEND_SIZE;
            printf("fp = %p ; a SEND_SIZE of\n", fp);
        }
	else{
            printf("send_size = %d ; a SEND_SIZE error\n", send_size);
        }
    }
...
void recv_fileinfo(int sockfd)
{
    ...
    //文件信息
    printf("------- fileinfo -------\n");
    printf("filename = %s\nfilesize = %d\nnum = %d\nbs = %d\n", finfo.filename, finfo.filesize, finfo.num, finfo.bs);
    printf("------------------------\n");
    ...
    for(int i=0;i<save_file_len;i++){
        if(svfile[i].used == 1 || svfile[i].used == 2)
            continue;
	strcpy(svfile[i].filename, finfo.filename);
    ...
    creatfile(filepath, finfo.filesize);
    int fd = 0;
...
void recv_filedata(int sockfd)
{
    ...
    if(gconn[recv_id].count == gconn[recv_id].num){
        ...
        for(int i=0;i<svfile_num;i++){
            if(gconn[recv_id].filename == svfile[i].filename){
                svfile[i].used == 2;
...
void send_fileinfo(int sockfd){
    ...
    for(int i=0;i<svfile_num;i++){
        if(svfile[i].used == 2){
	    strcpy(pfile->filename, svfile[i].filename);
...
{	
    ...
    /*最后一个分块可能不足一个标准分块*/
    int num = p_fstat->st_size/BLOCKSIZE;
    if(p_fstat->st_size%BLOCKSIZE == 0){
        p_finfo->num = num;
    }
    else{
        p_finfo->num = num+1;
        *p_last_bs = p_fstat->st_size - BLOCKSIZE*num;
    }
	...
	printf("-------- fileinfo -------\n");
    printf("filename= %s\nfilesize= %d\ncount= %d\nblocksize= %d\n", p_finfo->filename, p_finfo->filesize, p_finfo->num, p_finfo->bs);
	printf("-------------------------\n");
    return;
}
...
void send_filedata(int sockfd){
    ...
    close(opfd);
    int j = 0, num = sdfinfo.num, offset=0;
    if(last_bs == 0){
        for(j=0; j<num; j++){
